...
async function main() {
  ...
  // ====== To check if it will get included in the next block or not, you can use bundleResponse.wait() but for the sake of this tutorial, we will just wait patiently for a few blocks and observe..=======
  ...
  provider.on('block', async (blockNumber) => {
    ...
    const bundleResponse = await flashbotsProvider.sendBundle(
      [
        {
          transaction: {
            // ChainId number for the Goerli network
            chainId: 5,
            // EIP-1559 (because we will use the Post-London Upgrade gas model)
            type: 2,
            // Value of 1 FakeNFT
